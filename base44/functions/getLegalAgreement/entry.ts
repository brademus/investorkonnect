...
Deno.serve(async (req) => {
  try {
    ...
    // For INVESTORS: if this room has an agent with a regenerated counter agreement,
    // and the investor is the deal investor, show them the regenerated agreement
    // so they can complete signing. Only apply if room has a single active agent.
    if (!callerIsAgent && callerProfile && room_id) {
      const rooms = await base44.asServiceRole.entities.Room.filter({ id: room_id });
      const room = rooms?.[0];
      if (room && room.agent_terms) {
        // Find an agent whose counter was accepted and has a regenerated agreement
        for (const [agId, agTerms] of Object.entries(room.agent_terms)) {
          if (agTerms.regenerated_agreement_id && !agTerms.requires_regenerate) {
            ...
            if (regen && !['superseded', 'voided'].includes(regen.status)) {
              // Verify the investor owns this deal
              const deals = await base44.asServiceRole.entities.Deal.filter({ id: deal_id });
              if (deals?.[0]?.investor_id === callerProfile.id ||
                  room.investorId === callerProfile.id) {
                agreement = regen;
