...
export default function InlineReviewForm({ dealId, agentProfileId, reviewerProfileId, onSubmitted, compact = false }) {
  ...
  useEffect(() => {
    if (!reviewerProfileId || !agentProfileId || !dealId) return;
  ...
  }, [reviewerProfileId, agentProfileId, dealId]);

  // Subscribe to review updates in real-time
  useEffect(() => {
    if (!reviewerProfileId || !dealId || !agentProfileId) return;
    const unsub = base44.entities.Review.subscribe((event) => {
      if (event.data?.deal_id === dealId && 
          event.data?.reviewee_profile_id === agentProfileId &&
  ...
  }, [reviewerProfileId, dealId, agentProfileId]);
  ...
  return (
    <div className={`border border-[#1F1F1F] rounded-xl p-4 ${compact ? 'bg-[#0D0D0D]' : 'bg-[#141414]'}`}>
      <div className="mb-3">
        <p className={`font-semibold text-[#FAFAFA] mb-2 ${compact ? 'text-xs' : 'text-sm'}`}>{isEditing ? "Edit your review" : "Rate this agent"}</p>
