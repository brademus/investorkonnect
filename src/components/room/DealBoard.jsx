...
import { Info, Shield, FileText, Image, User, Plus, Download, Activity, Calendar, Check } from "lucide-react";
...
import { formatSlot } from "@/components/room/walkthroughActions";

/** Small header-level walkthrough badge */
function WalkthroughBadge({ deal }) {
  ...
  if (confirmed) {
    return (
      <div className="flex items-center gap-2 mt-3">
        ...
        <span className="text-sm font-medium text-[#34D399]">{formatSlot(confirmed)}</span>
      </div>
    );
  }
  ...
  return (
    <div className="mt-3 space-y-1.5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm text-[#808080]">Walk-through — Awaiting Confirmation</span>
      </div>
      <div className="space-y-1 ml-6">
        {slots.map((slot, idx) => {
          const timeLabel = [slot.timeStart, slot.timeEnd].filter(Boolean).join(' – ') || null;
          return (
            <div key={idx} className="flex items-center gap-2.5 p-2 bg-[#141414] rounded-lg border border-[#1F1F1F] text-xs">
              <Calendar className="w-3.5 h-3.5 text-[#E3C567] flex-shrink-0" />
              <span className="text-[#FAFAFA] font-medium">{slots.length > 1 ? `Option ${idx + 1}: ` : ''}{slot.date}</span>
...
export default function DealBoard({ deal, room, profile, roomId, onInvestorSigned, selectedAgentProfileId }) {
  ...
  const messagePhotos = useMemo(() =>
    (roomMessages || [])
      ...
      .map(m => ({ name: m.metadata.file_name || 'Photo', url: m.metadata.file_url, uploaded_by: m.sender_profile_id, uploaded_at: m.created_date, source: 'message' })),
    [roomMessages]
  );

  const [visitedTabs, setVisitedTabs] = useState(new Set(['details']));
  useEffect(() => { setVisitedTabs(prev => { if (prev.has(activeTab)) return prev; const next = new Set(prev); next.add(activeTab); return next; }); }, [activeTab]);
  ...
  const hasWalkthrough = (localDeal?.walkthrough_slots || []).filter(s => s.date && s.date.length >= 8).length > 0;
        ...
        <div className="space-y-6">
          {maskAddr && (
            <div className="bg-[#F59E0B]/10 border border-[#F59E0B]/30 rounded-2xl p-5 flex items-start gap-3">
              ...
              <div><h4 className="text-md font-bold text-[#F59E0B] mb-1">Limited Access</h4><p className="text-sm text-[#FAFAFA]/80">Full details unlock after both parties sign.</p></div>
            </div>
          )}
          {/* Deal Header */}
          <div className="bg-[#0D0D0D] border border-[#1F1F1F] rounded-2xl p-6">
            ...
            <WalkthroughBadge deal={localDeal} />
          </div>

          {/* Property Details + Key Terms */}
          ...
          {/* Walkthrough Schedule Modal */}
          <WalkthroughScheduleModal
            open={wtModalOpen} onOpenChange={setWtModalOpen}
            deal={localDeal} roomId={roomId} profile={profile}
            onScheduled={(updated) => setLocalDeal(prev => prev ? { ...prev, ...updated } : prev)}
          />
            ...
            <div className="space-y-3">
              {PIPELINE_STAGES.filter(s => s.id !== 'canceled').map(stage => {
                ...
                const handleStageClick = async () => {
                  ...
                  if (stage.id === 'new_deals' && norm !== 'new_deals') { toast.error("Deals cannot be moved back to New Deals."); return; }
                  if (!isSigned && stageOrder(stage.id) >= stageOrder('connected_deals')) { toast.error("Agreement must be fully signed before moving this deal forward."); return; }
                  ...
                  if (stage.id === 'completed' || stage.id === 'canceled') {
                    ...
                    if (agentId) navigate(`${createPageUrl("RateAgent")}?dealId=${localDeal.id}&agentProfileId=${agentId}&returnTo=Pipeline`);
                  }
                };
                return (
                  <div key={stage.id}>
                    ...
                    {isActive && (
                      <div className="ml-[52px] mt-2 mb-1 space-y-3">
                        <DealNextStepCTA
                          deal={localDeal} room={localRoom} profile={profile} roomId={roomId} inline
                          onDealUpdate={(patch) => {
                            if (!patch) return;
                            if (patch.documents) localDocsRef.current = { ...localDocsRef.current, ...patch.documents };
                            setLocalDeal(prev => {
                              if (!prev) return prev;
                        ...
                        />
                        {(stage.id === 'new_deals' || stage.id === 'connected_deals') && hasWalkthrough && (
