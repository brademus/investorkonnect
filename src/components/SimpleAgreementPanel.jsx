 ...
 */
export default function SimpleAgreementPanel({ dealId, roomId, profile, deal, onInvestorSigned, draftId, dealData, selectedAgentProfileId, agreement: externalAgreement, room: externalRoom, pendingCounters: externalPendingCounters, setPendingCounters: externalSetPendingCounters, inline = false }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const exhibit_a = {
        ...
        special_instructions: dealData?.notes || ''
      };
    ...
    try {
      ...
      if (role === 'agent') {
        ...
        if (!investorSignedAg) {
          ...
          investorSignedAg = allAgs.find(a =>
            a.investor_signed_at && a.docusign_envelope_id && !['superseded', 'voided'].includes(a.status)
          );
        }
              ...
              <div key={counter.id} className="bg-[#E3C567]/10 border border-[#E3C567]/30 rounded-xl p-4">
                ...
                {((counter.from_role === 'agent' && isInvestor) || (counter.from_role === 'investor' && isAgent)) && (
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" onClick={() => handleCounterResponse(counter.id, 'decline')} disabled={respondingCounterId === counter.id} variant="outline" className="flex-1 border-[#1F1F1F] bg-[#1A1A1A] hover:bg-[#222] text-[#FAFAFA] text-xs">Decline</Button>
                  </div>
                )}
    ...
    return (
      <div className="border-t border-[#1F1F1F] pt-5">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-base font-semibold text-[#FAFAFA]">Agreement</h4>
          {agreement && (
            <Badge className="bg-transparent border-[#1F1F1F]">
              {fullySigned ? <span className="text-green-400 flex items-center gap-1"><CheckCircle2 className="w-3 h-3" /> Signed</span>
                ...
                : needsRegen ? <span className="text-[#F59E0B] flex items-center gap-1"><Clock className="w-3 h-3" /> {!isInvestorDisplay ? 'Waiting for Investor' : 'Action Required'}</span>
                : investorSigned ? <span className="text-yellow-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {!isInvestorDisplay ? 'Ready to Sign' : 'Waiting for Agent'}</span>
                : <span className="text-blue-400 flex items-center gap-1"><Clock className="w-3 h-3" /> {!isInvestorDisplay ? 'Waiting for Investor' : 'Pending'}</span>}
            </Badge>
          )}
        </div>
        {agreementBody}
