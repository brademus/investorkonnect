      ...
      <div className="max-w-[700px] mx-auto px-4 pb-12">
        <div className="bg-[#0D0D0D] rounded-3xl p-12 border border-[#1F1F1F]" style={{ boxShadow: '0 6px 30px rgba(0,0,0,0.6)' }}>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-[#1F1F1F]">
            ...
            <button
              onClick={handleNext}
              disabled={saving || (step === 1 && (!formData.first_name.trim() || !formData.last_name.trim() || (formData.phone || '').replace(/\D/g, '').length < 10)) || (step === 2 && !formData.nationwide && (!formData.primary_states || formData.primary_states.length === 0)) || (step === 2 && (formData.deal_types || []).length === 0)}
