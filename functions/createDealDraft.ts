...
Deno.serve(async (req) => {
  try {
    ...
    const draft = await base44.asServiceRole.entities.DealDraft.create({
      ...
      number_of_signers,
      second_signer_name,
      seller_commission_type: seller_commission_type === 'flat' ? 'flat_fee' : seller_commission_type,
      seller_commission_percentage: seller_commission_type === 'percentage' ? Number(seller_commission_percentage) : null,
      seller_flat_fee: (seller_commission_type === 'flat_fee' || seller_commission_type === 'flat') ? Number(seller_flat_fee) : null,
      buyer_commission_type: buyer_commission_type === 'flat' ? 'flat_fee' : buyer_commission_type,
      buyer_commission_percentage: buyer_commission_type === 'percentage' ? Number(buyer_commission_percentage) : null,
      buyer_flat_fee: (buyer_commission_type === 'flat_fee' || buyer_commission_type === 'flat') ? Number(buyer_flat_fee) : null,
